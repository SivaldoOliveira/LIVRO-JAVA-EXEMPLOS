    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < registros.length; i++) {
            int tipo = showOptionDialog(null, "Tipo do funcionário", "Selecione", 0, QUESTION_MESSAGE, null,
                    new String[] { "Padrão", "Comissionado", "Produtividade" }, "Padrão");
            if (tipo == -1)
                break;
            if (tipo == 0)
                registros[i] = new FPadrao();
            else if (tipo == 1)
                registros[i] = new FComissionado();
            else if (tipo == 2)
                registros[i] = new FProdutividade();
        ...
        showMessageDialog(null, str, "Folha de Pagamento", INFORMATION_MESSAGE);
        System.exit(0);
